...
Shadowrocket покупается в **US App Store** за $2.99 (в российском его нет). Дальше схема работала так:
...
4. Итерация пока не заработает — без единой строчки вручную набранного текста на iPhone
...
У меня был инцидент: включил Force All Networks, рассуждая «звучит же логично, пусть VPN на всех сетях сразу». Shadowrocket крэшнулся с замершим экраном, VPN не отключался, iPhone на несколько минут завис в состоянии без интернета. Спасло только полный Reset настроек через меню. После этого правило простое: **если настройка не в списке «безопасных» — не трогать**, даже если она «звучит логично».
...
Это самая практичная часть, которую я хотел бы иметь перед началом. Если читатель — новичок, у него **нет работающего Claude Code** на локальной машине (Anthropic его не пустит из РФ). Ставить npm-пакет через заблокированный API тоже не получится. А мой скилл как раз и нужен, чтобы поставить VPN, — но чтобы запустить скилл, нужен Claude.
...
Меня задолбало платить 500-1000 ₽/мес за VPN, который раз в день выкидывает с 403 и ломает рабочий цикл. Каждый реконнект — это минут 10 выкинутого времени, и чем чаще — тем больнее.
