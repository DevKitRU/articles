...
Параллельно подкрался типичный гнилой момент: подписка на Claude Pro оплачена, а работать через публичный VPN нормально не получается. Логин в Anthropic слетает раз-два в день, каждый раз — это 5-10 минут выбитого из потока. Hiddify с «Russia bypass» решает половину: большинство сайтов открывается, но Mini App газсервис-бота в Telegram не грузится, STALCRAFT ругается на регион, Discord зависает на «Starting». Смена режимов (proxy / system / VPN) помогает одним приложениям и ломает другим. По ощущению — играешь в игру «угадай рабочий режим» без подсказок.
...
**Не пошло.** `sing-box` без admin не может создать TUN-интерфейс → падает с «Access denied». Через `Start-Process -Verb RunAs` внутри батника — UAC prompt при каждом логоне. Для эпизодического запуска ок, для автостарта «раз и забыл» не подходит.

### Способ 3: Windows Task Scheduler с `RunLevel=Highest` + `LocalSystem`

**Не пошло.** Scheduled Task падал с `LastTaskResult=1` на старте TUN-интерфейса. Несколько часов дебага, причина не гуглится — StackOverflow молчит, ChatGPT и Claude разводят руками, в event viewer только код без объяснения. Тупик. Подозрение — особенность DispatcherQueue/COM в TaskScheduler контексте. Плюс отдельный бонус — имя пользователя в кириллице («Администратор») через PowerShell `New-ScheduledTaskPrincipal -UserId 'Администратор'` ломается кодировкой, пришлось переходить на SID.

### Способ 4: Windows Service через WinSW (сработал, но с нюансом)
...
### Нюанс: после первого ребута служба не стартанула

Честно — **с первого раза после ребута sing-box не запустился сам**. Служба в статусе Stopped, интернет через VPN не идёт, и классическая засада: **чтобы починить VPN, нужен интернет**. Логов под рукой нет, гуглить не могу, на StackOverflow не зайти, сервис молча лежит.

Выкрутился через свой же VPS: подключился к нему по SSH (там Claude Code уже стоял, см. секцию 9), попросил Claude Code на VPS разобраться. Он положил рабочий конфиг на VPS по HTTP, продиктовал мне команду для Windows — скачать этот конфиг, пересоздать службу, прописать env-переменные, перезапустить. Я вставлял команды в PowerShell, он читал мой вывод в соседней сессии и корректировал.

После этого всё встало — и больше **не отваливалось ни разу** (проверено несколькими ребутами). То есть правильный конфиг + правильно установленная служба действительно работают «поставил-забыл», но первая установка на разных машинах может дать сюрпризы — полезно иметь backup-план.

Этот паттерн («удалённый Claude чинит локальную систему через SSH / копипаст команд») оказался удобнее чем мой исходный план ставить всё через локальный скрипт. В репо для таких случаев есть [docs/first-time-setup.md](https://github.com/DevKitRU/my-vpn-kit/blob/main/docs/first-time-setup.md) — подробно про секцию 9.

**Почему не Hiddify.** Он хорош как «поставил-забыл» для обычного пользователя, но: его встроенный роутинг не учитывает РФ-платформы на иностранных TLD (будет секция 8), custom rules в GUI не редактируются (только через кастомный sing-box XML, что обесценивает наличие Hiddify). Плюс Hiddify обновляется — в моменте поведение может поменяться. Для моего use-case (кастомные правила + долгосрочный control) — проще держать sing-box напрямую.
...
Причём **`sing-box check -c config.json` эту ошибку не ловит** — валидатор говорит OK, sing-box падает молча. Я успел погрешить на сеть, на конфиг VLESS, на DNS провайдера — оказался один параметр.

Правильно — без `detour`, sing-box сам решит:
...
## 7. iOS через Shadowrocket — буфер iCloud вместо ручного ввода

Никакого `install.sh` на iOS не выйдет — Apple не даёт программно настраивать VPN из скрипта. Но **ручками в iPhone тоже ничего вводить не надо**, если есть Mac рядом.

Shadowrocket покупается в **US App Store** за $2.99 (в российском App Store его нет, карта тоже нужна иностранная). Дальше схема работала так:

1. Claude на Mac (через Claude Code) пишет VLESS-ссылку с конкретными параметрами для Reality, копирует её в буфер Mac
2. iCloud синхронизирует буфер → на iPhone в Shadowrocket тыкаю «плюс» → **Импортировать из буфера** → профиль импортирован
3. Пробую подключиться; если что-то не работает (bad handshake, SNI не проходит) — возвращаюсь к Claude на Mac, **он меняет параметр в ссылке** (например, `fp=chrome` → `fp=firefox`, или другой `spx`), копирует новую версию → iCloud → новый импорт
4. Повторяю пока не заработает — без единой строчки вручную набранного текста на iPhone

Фактически Claude на Mac перебирал параметры VLESS-ссылки, я нажимал «Импорт» и смотрел результат. Удобство — весь debug через копипасту. После 3-4 итераций подбора настройка встала.

Базовые шаги уже после импорта рабочей ссылки:

- Shadowrocket → главный экран → нижняя плашка режимов → **Прокси** (Global)
- Включить переключатель VPN
...
У меня был инцидент: включил Force All Networks, рассуждая «звучит же логично, пусть VPN на всех сетях сразу». Shadowrocket крэшнулся с замершим экраном, VPN не отключался, iPhone на несколько минут остался без интернета вообще. Спасло только полный Reset настроек через меню. После этого правило простое: **если настройка не в списке «безопасных» — не трогать**, даже если она «звучит логично».
...
## 9. Claude-на-VPS — как я на самом деле всё настраивал

Это самая практичная часть, которую я хотел бы прочесть **до того как начать**. Если читатель — новичок, у него **нет работающего Claude Code** на локальной машине (Anthropic его не пустит из РФ). Ставить npm-пакет через заблокированный API тоже не получится. А мой скилл как раз и нужен, чтобы поставить VPN, — но чтобы запустить скилл, нужен Claude.

**Выход, который я использовал на всех трёх платформах:** Claude Code ставится не на локальную машину, а на **VPS**. У VPS non-RU IP, Anthropic пускает его без вопросов.
...
claude auth login   # ссылку OAuth открываешь в любом браузере
```

Дальше Claude на VPS:
- Выполняет скилл AndyShaman (поднимает 3x-ui на этом же VPS), получает VLESS
- Пишет конфиги sing-box для клиентской машины
- Кладёт их на VPS по HTTP (встроенный `python -m http.server 8888` — за 10 секунд) и даёт мне команду `iwr .../конфиг -o locally`
- Диктует команды в PowerShell / bash — я в своей локальной SSH-сессии или терминале вставляю копипастом
- Читает вывод моих команд, корректирует дальше

### Мой реальный порядок установки

1. **Mac (первый)** — через Claude на VPS. Бóльшая часть конфига, который потом пошёл в my-vpn-kit, была собрана именно тут. Claude на VPS сгенерил `config.json` → положил на VPS под `http://VPS:8888/singbox-mac.json` → я скачал на Mac одной командой.
2. **iPhone (с Mac)** — Shadowrocket настраивал сидя за Mac, где уже работал VPN. Claude на Mac генерил VLESS-ссылку с подбором параметров (Reality иногда капризничает по `fp` / `sni` / `spx`), копировал в буфер Mac → iCloud синхронизировал буфер → на iPhone «Импортировать из буфера» в Shadowrocket. Если первая ссылка не работала, Claude менял один параметр и отдавал новую версию. Всё через копипасту, на iPhone руками не вводил ни одной строки.
3. **Windows (в последнюю очередь)** — когда установка с нуля через мой же install.ps1 после первого ребута дала сбой (см. нюанс в секции 5), снова пошёл на VPS к Claude. Он смотрел логи службы, подобрал недостающие env-переменные и правильный путь к конфигу. Я вставлял — он читал результат. За 20 минут всё стартовало окончательно.

### Termius для удобства

Для SSH с iPhone использовал **Termius** — бесплатный SSH-клиент с нормальным GUI и синхронизацией ключей. Фактически всю инфраструктуру можно настроить **с телефона**: подключился к VPS, запустил Claude, получил команды, переключился на ноутбук, выполнил. Или через обратный SSH с VPS на локальную машину (если на Windows включён OpenSSH Server).

Этот подход экономит 2-3 часа новичку и избавляет от необходимости покупать временный публичный VPN только чтобы поставить постоянный свой.

Полный пошаговый гайд — в [docs/first-time-setup.md](https://github.com/DevKitRU/my-vpn-kit/blob/main/docs/first-time-setup.md) репо.

---

## 10. Что не получилось / не проверено

Для чистоты — список того, что я **не** сделал или не идеально:

- **Не закрыл SSH сразу после настройки VPS.** Самый болезненный урок из серии «потом закрою». AndyShaman в своём скилле 3x-ui прямо говорит: после установки — **отключить парольный вход SSH, оставить только по ключу**. Плюс хорошая практика — сменить порт с 22 на нестандартный. Я поленился («сначала пусть всё работает») — через несколько недель VPS начал периодически ловить массовый SSH-брутфорс с ботсетей, CPU скакал, мой собственный SSH-коннект падал посреди работы. В итоге закрутил, но лечить уже, а не предотвращать — это разница в два-три вечера времени. Правильный рецепт: как только ключ работает, **сразу** правишь `/etc/ssh/sshd_config` — `PasswordAuthentication no`, `Port XXXXX`, `systemctl restart sshd`, плюс поставить fail2ban. Делайте сразу, не как я.
...
- **TUN через Scheduled Task от SYSTEM** — падал в `LastTaskResult=1`. Не нашёл точную причину. Обошёл через Windows Service (WinSW) — там от LocalSystem TUN создаётся нормально.
...
- **Пинг до VPS ~180 мс** (OVH Канада). На работе с API / браузером это не заметно, но для шутеров с outbound через VPN будет чувствоваться. Решение — если играешь в игры через VPN, брать VPS в Европе или Турции; в моём случае игровые домены (STALCRAFT, Steam) идут direct с RU IP через split-tunneling, а 180 мс на Anthropic API — некритично.

### Что наоборот работает лучше чем ожидал

- **Discord Desktop** — на начальных попытках зависал на «Starting», я уже был готов отправлять его в Web. Но после финальной конфигурации (TUN + LocalSystem + правильные правила routing) всё ожило, Discord стартует нормально, голос работает. Видимо, проблема была в промежуточных итерациях с system proxy, а не в Electron как таковом.
- **Автозапуск** — после того как правильный конфиг подхватился, служба стартует до логина, стабильно переживает ребуты. Проверял несколько раз.
...
Меня задолбало платить 500-1000 ₽/мес за VPN, который раз в день выбрасывает с ошибкой 403 и ломает рабочий цикл. Каждый реконнект — это минут 10 выкинутого времени, и чем чаще — тем больнее.

Личный счётчик — **три разных платных VPN за год**, все покупались сразу на годовую подписку (потому что так выгоднее, если реклама не врёт). Каждый исправно работал первый месяц-два, потом что-то ломалось в их инфраструктуре: массово банили IP-подсети, меняли протокол под очередную РКН-блокировку, терялся стабильный exit. Писал в поддержку — тишина или отписки. Возвратов по неиспользованной части года не бывает. Суммарно около 6000 ₽ впустую, на три кривых опыта.

Свой VPS за 8000 ₽ / год — **и никакого third-party фактора между тобой и API**. Если что-то сломалось — смотришь логи своего sing-box, а не ждёшь когда провайдер починит.

Проект — не для денег (MIT, поддержка через issues). Хотелось, чтобы начинающему разработчику в РФ (каким я был ещё год назад) не приходилось тратить день на танцы с клиентами и антивирусами — и не сжигать по 2000 ₽ каждые пару месяцев на очередной «надёжный» VPN.
