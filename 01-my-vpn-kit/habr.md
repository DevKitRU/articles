...
- **Не закрыл SSH сразу после настройки VPS.** Это мой текущий самый болезненный урок. AndyShaman в своём скилле 3x-ui прямо говорит: после установки — **отключить парольный вход SSH, оставить только по ключу**. Плюс хорошая практика — сменить порт с 22 на нестандартный. Я поленился («потом закрою, сначала пусть всё работает») — теперь VPS периодически ловит массовый SSH-брутфорс с ботсетей, CPU скачет, мой собственный SSH-коннект падает посреди работы. Как только ключ работает, **сразу** правишь `/etc/ssh/sshd_config`: `PasswordAuthentication no` + `Port XXXXX` + `systemctl restart sshd`. Поздняя настройка = лечить боль, а ранняя = предотвратить её совсем. **Делайте сразу, не как я.**
