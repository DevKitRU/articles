...
Честно — **с первого раза после ребута sing-box не запустился сам**. Служба была в статусе Stopped, конфиг почему-то не подтянулся, интернет через VPN не шёл. Что произошло, я с ходу не понял — логов на руках не было, чинить с локальной машины, у которой интернет лежит, не получалось.
